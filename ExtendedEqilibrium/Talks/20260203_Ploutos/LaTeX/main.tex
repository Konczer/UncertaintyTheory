...
\date{\href{https://ploutos.dev}{Ploutos}  \\ Online
\\
\
\\
3 February 2026}
...
\begin{frame}{}
    \centering
    \includegraphics[width=0.3\textwidth]{img/EEQR.pdf}
    
    \vspace{1cm}
    
    \href{https://arxiv.org/abs/2503.01889}{\bf arXiv:2503.01889}
\end{frame}
...
\begin{frame}{Generals and the Weather }
\centering
    \includegraphics[angle=90, width=0.9\textwidth]{img/Gw.pdf}
...
A pair of the Players' -- potentially mixed -- strategy and their subjective believes:
...
\begin{frame}{Regret over negative utility}

\centering

The ``Bear story'' from 

the Essay on uncertainty

\includegraphics[width=0.2\textwidth]{img/EssayQR.pdf}
    ...
    Utility matrix of the band. Utilities are estimated in utils (denoted as $\util$).}
...
\begin{table}[h!]
    ...
    \begin{tabular}{c|cc}
        Band $\backslash$ Bear & Present & Absent \\
        \hline
        Prepared but slow & \makecell{Best action \\ if the bear is Present ($0\ \util$)}   & \makecell{Inconvenient \\ instead of Convenient ($11\ \util$)} \\
        Swift but unprepared & \makecell{Very bad \\ instead of Bad ($40\ \util$)} & \makecell{Best action \\ if the bear is Absent ($0\ \util$)} \\
    \end{tabular}
    \caption{
    \fontsize{8pt}{7.2}\selectfont
    Regret matrix of the band. Regrets are determined as 
    $R(a,\theta) = \max_{b} U(b,\theta) - U(a,\theta)$, 
    \mbox{$\theta \in \{\mathrm{Present},\mathrm{Absent}\}$}, 
    \mbox{$a,b \in \{\mathrm{Prepared},\mathrm{Unprepared}\}$}; and are measured in the same utils as utility (denoted as $\util$).}
...
\begin{quote}
    ``If a man will begin with certainties, he shall end with doubts, but if he will be content to begin with doubts he shall end in certainties'' --- \href{https://www.oxfordreference.com/display/10.1093/acref/9780191826719.001.0001/q-oro-ed4-00003780}{Francis Bacon The Advancement of Learning (1605)}
\end{quote}
...
\begin{frame}{}
    \centering
    \includegraphics[width=0.65\textwidth]{img/pqPlotAnnotated.png}

    Set of Bayesian game solutions versus the Extended Equilibrium
\end{frame}
...
\begin{frame}{}
    \begin{itemize}
        \item Extended Equilibrium GitHub repo
        \includegraphics[width=0.2\textwidth]{img/EEGitHubQR.pdf}
        \item \includegraphics[width=0.2\textwidth]{img/WCPQR.pdf} Wolfram Community post
        \item Extended Blog post on Statistical Games
        \includegraphics[width=0.2\textwidth]{img/SGBlogQR.pdf}
    \end{itemize}
\end{frame}
